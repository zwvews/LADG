...
def train(algorithm, datasets, general_logger, config, epoch_offset, best_val_metric):
    bestResultsVal = None
    bestResultsTest = None
    bestResultsTestIn = None
    for epoch in range(epoch_offset, config.n_epochs):
        ...
        if algorithm.update_count > 0:
            ...
            for split in additional_splits:
                if split=='test':
                    results_Test, y_pred = run_epoch(algorithm, datasets[split], general_logger, epoch, config, train=False)
                else:
                    if config.dataset=='iwildcam' and split=='id_test':
                        results_TestIn, y_pred = run_epoch(algorithm, datasets[split], general_logger, epoch, config, train=False)
                    elif config.dataset=='camelyon17' and split=='id_val':
                        results_ValIn, y_pred = run_epoch(algorithm, datasets[split], general_logger, epoch, config, train=False)
                    else:
                        _, y_pred= run_epoch(algorithm, datasets[split], general_logger, epoch, config, train=False)

            if bestResultsVal is None or is_best:
                bestResultsVal = val_results

            if bestResultsTest is None or is_best:
                bestResultsTest = results_Test

            if config.dataset == 'iwildcam':
                if bestResultsTestIn is None or is_best:
                    bestResultsTestIn = results_TestIn

            dict_wandb = {}
            for k,v in bestResultsTest.items():
                k = 'bestTest/' + k
                dict_wandb[k] = v
            for k,v in bestResultsVal.items():
                k = 'bestVal/' + k
                dict_wandb[k] = v
            if config.dataset == 'iwildcam':
                for k, v in bestResultsTestIn.items():
                    k = 'bestTestIn/' + k
                    dict_wandb[k] = v
